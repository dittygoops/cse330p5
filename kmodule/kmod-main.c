...
// static struct bio *usb_bio = NULL; // Consider if needed globally or per-function
static struct file *usb_file = NULL;
static struct bio_set *usb_bio_set = NULL; // Required for bio_alloc in modern kernels
...
static long rw_usb(char* data, unsigned int size, unsigned int offset, bool flag); // Made static
...
static bool open_usb(void)
{
    ...
    // Create a bio_set - required for bio_alloc
    ...
    // No need to pre-allocate usb_bio here, can do it in rw_usb
    // usb_bio = bio_alloc(GFP_KERNEL, 1); // Allocate 1 vector (can hold multiple pages)
    // if (!usb_bio) {
    //    printk(KERN_ERR "kmod_main: error: failed to allocate a bio structure.\n");
    ...
    //    return false;
    // }
    // printk(KERN_INFO "kmod_main: success: allocated usb_bio.\n"); // KERN_INFO level
...
)
{
    ...
    if (offset == (unsigned int)-1) {
        ...
        current_sector = offset / 512; // Convert byte offset to sector offset [cite: 51]
        ...
        // No need to reset offset variable here, current_sector tracks the start
         // Update cur_dev_sector to the new starting position [cite: 48, 49, 50]
    ...
    while (remaining > 0) {
         ...
         // Allocate a new bio for each chunk (simpler resource management)
         // Allocate enough bio vecs for the chunk size (1 vec can hold PAGE_SIZE)
         // Since chunk is 512, 1 vec is sufficient.
         bio = bio_alloc_bioset(GFP_KERNEL, 1, usb_bio_set); // Allocate from our bioset
         if (!bio) {
             printk(KERN_ERR "rw_usb: error: failed to allocate bio for chunk.\n");
             mutex_unlock(&kmod_usb_mutex);
             return -ENOMEM;
         }

        /* Set the BIO structure */
         // Use bio_set_dev() to assign bio to bdevice [cite: 10] (Pseudocode line 11 [cite: 43])
        ...
        if (flag == true) {
            bio->bi_opf = REQ_OP_WRITE | REQ_SYNC; // Add REQ_SYNC for synchronous write
             printk(KERN_DEBUG "rw_usb: WRITE chunk (remaining=%u, sector=%llu)\n", // KERN_DEBUG for verbose logs
                   remaining, bio->bi_iter.bi_sector);
        } else {
            bio->bi_opf = REQ_OP_READ;
             printk(KERN_DEBUG "rw_usb: READ chunk (remaining=%u, sector=%llu)\n",
                   remaining, bio->bi_iter.bi_sector);
        }

        // Determine size for this chunk (max 512 bytes as per PDF note [cite: 39])
         bytes_in_chunk = (remaining > 512) ? 512 : remaining;

        // Add data page(s) to the bio
        // Need to handle cases where data spans page boundaries within the chunk if chunk > PAGE_SIZE
        // For 512 byte chunks, it's simpler.
        struct page *data_page = vmalloc_to_page(data + processed); // Get page for the current data offset [cite: 41]
        if (!data_page) {
        ...
        unsigned int offset_in_page = offset_in_page(data + processed);
         ...
         // Add page to BIO [cite: 35, 36]
         // bio_add_page(bio, data_page, bytes_in_chunk, offset_in_page); // Use offset within the vmalloced page
        ...
        /* Submit BIO and wait for op completion */
         ...
         bio_ret = submit_bio_wait(bio); // Returns 0 on success, < 0 on error [cite: 42]
     ...
     if (offset == (unsigned int)-1 && total_processed == size) {
         cur_dev_sector = current_sector; // Update global sector offset [cite: 45, 47]
         printk(KERN_INFO "rw_usb: Updated cur_dev_sector to %u\n", cur_dev_sector);
     } else if (offset != (unsigned int)-1 && total_processed == size) {
          // If a specific offset was provided, cur_dev_sector was already updated at the start [cite: 50]
...
static void close_usb(void)
{
    ...
    // No need to free usb_bio if allocated/freed within rw_usb
